...
from bitget import rest
from bitget.configuration import Configuration
from bitget.exceptions import ApiTypeError, ApiValueError
from bitget.schemas import (
    NoneClass,
    BoolClass,
    Schema,
    FileIO,
...
class ApiClient:
    ...
    def update_params_for_auth(self, headers, auth_settings,
        ...
        SECRET_KEY = self.configuration.auth_settings().get("SECRET_KEY")
        timestamp = utils.get_timestamp()
        if not body:
            body = bytes("", encoding = "utf8")
        sign = utils.sign(utils.pre_hash(timestamp, method, resource_path, str(body, 'utf-8')), SECRET_KEY['value'])
        headers.update({'ACCESS-TIMESTAMP': str(timestamp)})
        headers.update({'ACCESS-SIGN': str(sign, 'utf-8')})
        headers.update({'SECRET-KEY': ''})
