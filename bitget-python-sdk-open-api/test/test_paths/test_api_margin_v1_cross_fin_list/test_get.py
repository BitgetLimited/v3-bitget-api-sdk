...
import bitget
from bitget.paths.api_margin_v1_cross_fin_list import get  # noqa: E501
from bitget import configuration, schemas, api_client

from .. import ApiTestMixin


class TestApiMarginV1CrossFinList(ApiTestMixin, unittest.TestCase):
    """
    ApiMarginV1CrossFinList unit test stubs
        list  # noqa: E501
    """
        ...
        with bitget.ApiClient(configuration) as api_client:
            ...
            try:
                req = {
                    ...
                    'endTime': "1679833422000",
                }
                api_response = api_instance.cross_fin_list(req)
                print(api_response)
                ...
                for item in api_response.body['data']['resultList']:
                    print(item)
                    self.assertIsNotNone(item['marginId'])
                    self.assertIsNotNone(item['coin'])
                    self.assertIsNotNone(item['marginType'])
                    self.assertIsNotNone(item['balance'])
