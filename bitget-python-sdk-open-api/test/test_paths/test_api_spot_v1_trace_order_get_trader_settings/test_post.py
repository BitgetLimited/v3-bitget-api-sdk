...
import bitget
from bitget.paths.api_spot_v1_trace_order_get_trader_settings import post  # noqa: E501
from bitget import configuration, schemas, api_client

from .. import ApiTestMixin


class TestApiSpotV1TraceOrderGetTraderSettings(ApiTestMixin, unittest.TestCase):
    """
    ApiSpotV1TraceOrderGetTraderSettings unit test stubs
        getTraderSettings  # noqa: E501
    """
    ...
    def testApi(self):
        ...
        with bitget.ApiClient(configuration) as api_client:
            ...
            try:
                ...
                self.assertIsNotNone(api_response.body['data']['supportProductCodes'])
                for item in api_response.body['data']['supportProductCodes']:
                    print(item)
                    self.assertIsNotNone(item['openCopyTrace'])
                    self.assertIsNotNone(item['productCode'])
                    self.assertIsNotNone(item['productName'])
