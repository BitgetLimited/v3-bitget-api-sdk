...
import bitget
from bitget.paths.api_margin_v1_isolated_public_tier_data import get  # noqa: E501
from bitget import configuration, schemas, api_client

from .. import ApiTestMixin


class TestApiMarginV1IsolatedPublicTierData(ApiTestMixin, unittest.TestCase):
    """
    ApiMarginV1IsolatedPublicTierData unit test stubs
        tierData  # noqa: E501
    """
    ...
    def testApi(self):
        ...
        with bitget.ApiClient(configuration) as api_client:
            ...
            api_instance = margin_isolated_public_api.MarginIsolatedPublicTierData(api_client)
            try:
                req = {
                    'symbol':"BTCUSDT"
                }
                api_response = api_instance.margin_isolated_public_tier_data(req)
                print(api_response)
                ...
                for item in api_response.body['data']:
                    ...
                    self.assertIsNotNone(item['quoteMaxBorrowableAmount'])
                    self.assertIsNotNone(item['maintainMarginRate'])
                    self.assertIsNotNone(item['initRate'])
