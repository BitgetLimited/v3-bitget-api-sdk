...
import bitget
from bitget.paths.api_spot_v1_trace_order_my_tracers import post  # noqa: E501
from bitget import configuration, schemas, api_client

from .. import ApiTestMixin


class TestApiSpotV1TraceOrderMyTracers(ApiTestMixin, unittest.TestCase):
    """
    ApiSpotV1TraceOrderMyTracers unit test stubs
        myTracers  # noqa: E501
    """
    ...
    def testApi(self):
        ...
        with bitget.ApiClient(configuration) as api_client:
            ...
            try:
                ...
                req = MyTracersRequest(
                    pageSize=10
                )
                api_response = api_instance.spot_trace_my_tracers(req)
                print(api_response)
                ...
                for item in api_response.body['data']['resultList']:
                    print(item)
                    self.assertIsNotNone(item['accountEquity'])
                    self.assertIsNotNone(item['tracerNickName'])
                    self.assertIsNotNone(item['tracerUserId'])
