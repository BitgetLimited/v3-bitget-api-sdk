...
import bitget
from bitget.paths.api_spot_v1_trace_profit_profit_his_detail_list import post  # noqa: E501
from bitget import configuration, schemas, api_client

from .. import ApiTestMixin


class TestApiSpotV1TraceProfitProfitHisDetailList(ApiTestMixin, unittest.TestCase):
    """
    ApiSpotV1TraceProfitProfitHisDetailList unit test stubs
        profitHisDetailList  # noqa: E501
    """
    ...
    def testApi(self):
        ...
        with bitget.ApiClient(configuration) as api_client:
            ...
            try:
                ...
                req = TotalProfitHisDetailListRequest(
                    date= "1681985100000",
                    coinName= "USDT",
                )
                api_response = api_instance.spot_trace_profit_his_detail_list(req)
                print(api_response)
                ...
                for item in api_response.body['data']['resultList']:
                    print(item)
                    self.assertIsNotNone(item['coinName'])
                    self.assertIsNotNone(item['profit'])
                    self.assertIsNotNone(item['tracerNickName'])
