...
import { ApiResponseResultOfMarginIsolatedLoanInfoResult } from '../model/apiResponseResultOfMarginIsolatedLoanInfoResult';
...
export enum MarginIsolatedBorrowApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginIsolatedBorrowApi {
    ...
    public setApiKey(key: MarginIsolatedBorrowApiApiKeys, value: string) {
        (this.authentications as any)[MarginIsolatedBorrowApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async isolatedLoanList (symbol: string, startTime: string, coin?: string, endTime?: string, loanId?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedLoanInfoResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/isolated/loan/list';
        ...
        if (symbol === null || symbol === undefined) {
            throw new Error('Required parameter symbol was null or undefined when calling isolatedLoanList.');
        }

        // verify required parameter 'startTime' is not null or undefined
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling isolatedLoanList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedLoanInfoResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginIsolatedLoanInfoResult");
