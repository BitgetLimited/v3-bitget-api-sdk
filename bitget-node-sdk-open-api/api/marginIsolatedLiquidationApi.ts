...
import { ApiResponseResultOfMarginIsolatedLiquidationInfoResult } from '../model/apiResponseResultOfMarginIsolatedLiquidationInfoResult';
...
export enum MarginIsolatedLiquidationApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginIsolatedLiquidationApi {
    ...
    public setApiKey(key: MarginIsolatedLiquidationApiApiKeys, value: string) {
        (this.authentications as any)[MarginIsolatedLiquidationApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async isolatedLiquidationList (symbol: string, startTime: string, endTime?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedLiquidationInfoResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/isolated/liquidation/list';
        ...
        if (symbol === null || symbol === undefined) {
            throw new Error('Required parameter symbol was null or undefined when calling isolatedLiquidationList.');
        }

        // verify required parameter 'startTime' is not null or undefined
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling isolatedLiquidationList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedLiquidationInfoResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginIsolatedLiquidationInfoResult");
