...
import { ApiResponseResultOfMarginCrossFinFlowResult } from '../model/apiResponseResultOfMarginCrossFinFlowResult';
...
export enum MarginCrossFinflowApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginCrossFinflowApi {
    ...
    public setApiKey(key: MarginCrossFinflowApiApiKeys, value: string) {
        (this.authentications as any)[MarginCrossFinflowApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async crossFinList (startTime: string, coin?: string, endTime?: string, marginType?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginCrossFinFlowResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/cross/fin/list';
        ...
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling crossFinList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginCrossFinFlowResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginCrossFinFlowResult");
