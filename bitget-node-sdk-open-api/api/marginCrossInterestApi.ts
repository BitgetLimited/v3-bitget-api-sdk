...
import { ApiResponseResultOfMarginInterestInfoResult } from '../model/apiResponseResultOfMarginInterestInfoResult';
...
export enum MarginCrossInterestApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginCrossInterestApi {
    ...
    public setApiKey(key: MarginCrossInterestApiApiKeys, value: string) {
        (this.authentications as any)[MarginCrossInterestApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async crossInterestList (startTime: string, coin?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginInterestInfoResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/cross/interest/list';
        ...
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling crossInterestList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginInterestInfoResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginInterestInfoResult");
