...
import { ApiResponseResultOfMarginRepayInfoResult } from '../model/apiResponseResultOfMarginRepayInfoResult';
...
export enum MarginCrossRepayApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginCrossRepayApi {
    ...
    public setApiKey(key: MarginCrossRepayApiApiKeys, value: string) {
        (this.authentications as any)[MarginCrossRepayApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async crossRepayList (startTime: string, coin?: string, repayId?: string, endTime?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginRepayInfoResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/cross/repay/list';
        ...
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling crossRepayList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginRepayInfoResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginRepayInfoResult");
