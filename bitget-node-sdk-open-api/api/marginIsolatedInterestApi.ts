...
import { ApiResponseResultOfMarginIsolatedInterestInfoResult } from '../model/apiResponseResultOfMarginIsolatedInterestInfoResult';
...
export enum MarginIsolatedInterestApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginIsolatedInterestApi {
    ...
    public setApiKey(key: MarginIsolatedInterestApiApiKeys, value: string) {
        (this.authentications as any)[MarginIsolatedInterestApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async isolatedInterestList (symbol: string, startTime: string, coin?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedInterestInfoResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/isolated/interest/list';
        ...
        if (symbol === null || symbol === undefined) {
            throw new Error('Required parameter symbol was null or undefined when calling isolatedInterestList.');
        }

        // verify required parameter 'startTime' is not null or undefined
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling isolatedInterestList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedInterestInfoResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginIsolatedInterestInfoResult");
