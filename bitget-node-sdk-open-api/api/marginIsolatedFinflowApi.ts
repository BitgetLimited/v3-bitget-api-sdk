...
import { ApiResponseResultOfMarginIsolatedFinFlowResult } from '../model/apiResponseResultOfMarginIsolatedFinFlowResult';
...
export enum MarginIsolatedFinflowApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginIsolatedFinflowApi {
    ...
    public setApiKey(key: MarginIsolatedFinflowApiApiKeys, value: string) {
        (this.authentications as any)[MarginIsolatedFinflowApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async isolatedFinList (symbol: string, startTime: string, coin?: string, marginType?: string, endTime?: string, loanId?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedFinFlowResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/isolated/fin/list';
        ...
        if (symbol === null || symbol === undefined) {
            throw new Error('Required parameter symbol was null or undefined when calling isolatedFinList.');
        }

        // verify required parameter 'startTime' is not null or undefined
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling isolatedFinList.');
        }
        ...
        if (marginType !== undefined) {
            localVarQueryParameters['marginType'] = ObjectSerializer.serialize(marginType, "string");
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedFinFlowResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginIsolatedFinFlowResult");
