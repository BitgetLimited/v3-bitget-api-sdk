...
import { ApiResponseResultOfMarginIsolatedRepayInfoResult } from '../model/apiResponseResultOfMarginIsolatedRepayInfoResult';
...
export enum MarginIsolatedRepayApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginIsolatedRepayApi {
    ...
    public setApiKey(key: MarginIsolatedRepayApiApiKeys, value: string) {
        (this.authentications as any)[MarginIsolatedRepayApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async isolateRepayList (symbol: string, startTime: string, coin?: string, repayId?: string, endTime?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedRepayInfoResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/isolated/repay/list';
        ...
        if (symbol === null || symbol === undefined) {
            throw new Error('Required parameter symbol was null or undefined when calling isolateRepayList.');
        }

        // verify required parameter 'startTime' is not null or undefined
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling isolateRepayList.');
        }
        ...
        if (repayId !== undefined) {
            localVarQueryParameters['repayId'] = ObjectSerializer.serialize(repayId, "string");
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginIsolatedRepayInfoResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginIsolatedRepayInfoResult");
