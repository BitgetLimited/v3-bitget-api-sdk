...
import { ApiResponseResultOfMarginLiquidationInfoResult } from '../model/apiResponseResultOfMarginLiquidationInfoResult';
...
export enum MarginCrossLiquidationApiApiKeys {
    ACCESS_KEY,
    ACCESS_PASSPHRASE,
    ACCESS_SIGN,
    ACCESS_TIMESTAMP,
...
export class MarginCrossLiquidationApi {
    ...
    public setApiKey(key: MarginCrossLiquidationApiApiKeys, value: string) {
        (this.authentications as any)[MarginCrossLiquidationApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param pageId pageId
     */
    public async crossLiquidationList (startTime: string, endTime?: string, pageSize?: string, pageId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginLiquidationInfoResult;  }> {
        const localVarPath = this.basePath + '/api/margin/v1/cross/liquidation/list';
        ...
        if (startTime === null || startTime === undefined) {
            throw new Error('Required parameter startTime was null or undefined when calling crossLiquidationList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ApiResponseResultOfMarginLiquidationInfoResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ApiResponseResultOfMarginLiquidationInfoResult");
