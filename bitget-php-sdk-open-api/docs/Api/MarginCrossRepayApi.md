...
```php
crossRepayList($start_time, $coin, $repay_id, $end_time, $page_size, $page_id): \Bitget\Model\ApiResponseResultOfMarginRepayInfoResult
```
...
$apiInstance = new Bitget\Api\MarginCrossRepayApi(
...
$repay_id = 32428347234; // string | repayId
$end_time = 1678193338000; // string | endTime
$page_size = 10; // string | pageSize
$page_id = minId; // string | pageId

try {
    $result = $apiInstance->crossRepayList($start_time, $coin, $repay_id, $end_time, $page_size, $page_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling MarginCrossRepayApi->crossRepayList: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\Bitget\Model\ApiResponseResultOfMarginRepayInfoResult**](../Model/ApiResponseResultOfMarginRepayInfoResult.md)
