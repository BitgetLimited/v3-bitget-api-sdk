...
```php
crossInterestList($start_time, $coin, $page_size, $page_id): \Bitget\Model\ApiResponseResultOfMarginInterestInfoResult
```
...
$apiInstance = new Bitget\Api\MarginCrossInterestApi(
...
try {
    $result = $apiInstance->crossInterestList($start_time, $coin, $page_size, $page_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling MarginCrossInterestApi->crossInterestList: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\Bitget\Model\ApiResponseResultOfMarginInterestInfoResult**](../Model/ApiResponseResultOfMarginInterestInfoResult.md)
