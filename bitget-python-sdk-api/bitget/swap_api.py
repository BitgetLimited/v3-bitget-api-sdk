...
class SwapAPI(Client):
    ...
    def get_current_Track(self,symbol,pageIndex,pageSize):
        '''
        获取交易员当前带单list
        method : GET
        参数名	参数类型	是否必须	描述
        :param symbol: String 是 合约名称
        :param pageIndex: String 是 页码
        :param pageSize:String 是 数量
        :return:
        '''
        params = {}
        if symbol and pageSize and pageIndex:
            params["symbol"] = symbol
            params["pageSize"] = pageSize
            params["pageIndex"] = pageIndex
            return self._request_with_params(GET, API_TRACE + '/currentTrack', params)
        else:
            return "pls check args"



    def get_history_Track(self,pageIndex,pageSize,createDate):
        '''
        获取历史带单列表
        method : GET
        参数名	参数类型	是否必须	描述
        :param pageIndex: String 是 页码
        :param pageSize: String 是 数量
        :param createDate: Integer 是 限制天数(从当前时间开始往前推移）最大90天
        :return:
        '''
        params = {}
        if pageSize and pageIndex and createDate:
            params["pageSize"] = pageSize
            params["pageIndex"] = pageIndex
            params["createDate"] = createDate
            return self._request_with_params(GET,API_TRACE+'/historyTrack',params)
        else:
            return "pls check args"